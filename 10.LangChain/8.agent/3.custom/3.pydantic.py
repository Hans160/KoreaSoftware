...
class SendEmailInput(BaseModel):
    """이메일 전송 도구의 인자"""
    to: str = Field(..., description="이메일 수신자 주소(반드시 유효한 이메일 형식)")
...
def send_email(to:str, subject: str, body:str, priority:Literal["low", "normal", "high"] = "normal") -> str:
    """사용자가 요청할때 이메일을 보낸다."""
    print(f"[가짜 전송] to={to}, priority={priority}")
    print(f"제목: {subject}")
    print(f"본문: {body}")
    return f"이메일이 {to}로 전송되었습니다. (우선순위: {priority})"

class SearchInput(BaseModel):
    """검색 도구의 인자"""
    query: str = Field(description="검색어 (한국어로 작성)")
...
questions = [
    "alice@example.com 이게 회의 일정 변경이라는 제목으로 메일을 보내줘. 본문은 '회의가 내일 3시로 변경되었습니다.'로 보내고, 긴급해.",
...
SYSTEM = (
    "도구는 적합할 때만 사용하세요. 그리고 입력 인자들을 잘 확인하고, 오류가 없도록 호출하시오. 호출이후 오류가 발생한 경우 도구의 목적과 인자값을 잘 확인하고 재시도 하시오. 그러나 재시도는 최대 2회만 하시오. 적합한 도구를 발견하지 못했을경우,'해당 작업을 수행할수 있는 도구가 없습니다' 라고 답변한다."
)
